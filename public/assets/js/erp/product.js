...
layui.use(['table', 'element', 'form'], function () {
    ...
    table.on('tool(listForm)', function (obj) {
        var data = obj.data;
        if (obj.event === 'edit') {
            saveProduct('update', data.goods_id);
        }
    });
    var $ = layui.$, active = {
        searchData: function () {
        ...
        if (data.value > 0) {
            $.get(base_url + 'erp/pm/keyword/get', {key_id: data.value}, function (res) {
                if (res.status) {
                    ...
                    content += '<select name="product_keyword" lay-verify="required" lay-search="" multiple>';
     ...
     * @param product_id
     */
    function saveProduct(type, product_id) {
        var title = '新增商品';
        if (type == 'update') {
            title = '更新商品';
            type += '?product_id=' + product_id;
        }
        $.get(base_url + '/erp/pm/product/' + type, null, function (result) {
            if (!result.status) {
                ...
                layer.open({
                    ...
                    yes: function () {
                        ...
                        form.on('submit(edit)', function (data) {
                            $.ajax({
                                type: 'POST',
                                url: base_url + '/erp/pm/product/' + type,
                                data: $("form").serialize(),
                                dataType: 'json',
